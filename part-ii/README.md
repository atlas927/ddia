...
在本书的[第一部分](../part-i/)中，我们讨论了数据系统的各个方面，但仅限于数据存储在单台机器上的情况。现在我们到了[第二部分](./)，进入更高的层次，并提出一个问题：如果**多台机器**参与数据的存储和检索，会发生什么？
...
另一种方法是**共享磁盘架构（shared-disk architecture）**，它使用多台具有独立处理器和内存的机器，但将数据存储在机器之间共享的磁盘阵列上，这些磁盘通过快速网络连接。这种架构用于某些数据仓库，但竞争和锁定的开销限制了共享磁盘方法的可扩展性【2】。
...
复制和分区是不同的机制，但它们经常同时使用。如[图II-1](https://github.com/atlas927/ddia/tree/315c42dec3e9b1106dce0f2bcd97c222da7f08ac/img/figii-1.png)所示。

![](../.gitbook/assets/figii-1.png)
...
| 上一章 | 目录 | 下一章 |
| :--- | :--- | :--- |
| [第四章：编码与演化](../part-i/ch4.md) | [设计数据密集型应用](../) | [第五章：复制](ch5.md) |
